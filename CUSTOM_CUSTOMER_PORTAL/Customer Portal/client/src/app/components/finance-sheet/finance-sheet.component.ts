...
export class FinanceSheetComponent implements OnInit {
  ...
  loadFinancialData(): void {
    ...
    forkJoin({
      invoices: this.sapApi.getInvoices(customerId),
      aging: this.sapApi.getAging(customerId),
      memos: this.sapApi.getMemos(customerId)
    }).subscribe({
      next: (responses) => {
        ...
        const invoiceItems = this.getSafeItemArray(rawInvoices, 'ET_INVOICE');
        const agingItems = this.getSafeItemArray(rawAging, 'ET_PAYMENTS_AGING');
        const memoItems = this.getSafeItemArray(rawMemos, 'ET_MEMO');

        // Flatten and map SAP keys -> friendly keys
        this.invoices = invoiceItems.map((rec: any) => this.mapInvoice(this.flattenRecord(rec)));
        this.aging    = agingItems.map((rec: any) => this.mapAging(this.flattenRecord(rec)));
        this.memos    = memoItems.map((rec: any) => this.mapMemo(this.flattenRecord(rec)));
  ...
  private mapInvoice(rec: any): any {
    return {
      invoiceNumber: rec.VBELN ?? rec.VBELN_VL ?? null,
      invoiceDate: rec.FKDAT ?? rec.ERDAT ?? null,
      amount: rec.NETWR ?? null,
      currency: rec.WAERK ?? null,
      status: rec.STATUS ?? null,
      // keep original SAP for debugging if needed
      __raw: rec
    };
  }
  ...
  private mapAging(rec: any): any {
    return {
      invoiceNumber: rec.VBELN ?? null,
      invoiceDate: rec.FKDAT ?? rec.ERDAT ?? null,
      amount: rec.NETWR ?? null,
      currency: rec.WAERK ?? null,
      agingDays: rec.AGING ?? null,
      dueDate: rec.DATS ?? null,
      __raw: rec
    };
  }
  ...
  private mapMemo(rec: any): any {
    return {
      memoNumber: rec.VBELN ?? null,
      memoDate: rec.FKDAT ?? null,
      amount: rec.NETWR ?? null,
      type: rec.MEMO_TYPE ?? rec.FKART ?? null,
      currency: rec.WAERK ?? null,
      __raw: rec
    };
  }
